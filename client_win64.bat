...
set server_url="http://ajwest.dyndns.org:5001/pattern"
...
for /f "delims=" %%i in ('CALL "./dependencies/curl/curl" %server_url%') do set search_pattern=%%i
...
for /f "delims=" %%i in ('CALL "./dependencies/curl/curl" %server_url%') do set public_key=%%i
